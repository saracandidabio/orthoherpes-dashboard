...
MANIFEST_FILE = DATA_DIR / "dataset_manifest_v7.json"
...
def data_version() -> str:
    ...
    for path in (HOST_CROSSWALK_FILE, BUILD_AUDIT_FILE, MANIFEST_FILE, INTEREST_FILE, QUALITY_FILE,
...
def scope_note(filtered: bool = True):
    if filtered:
        st.caption("🔎 Esta seção respeita os filtros globais da barra lateral.")
    else:
        st.info(
            "Esta seção mostra dados pré-calculados no build e **ignora** "
            "os filtros globais."
        )
...
def ranked_bar(table, category, value, *, key, title, label, color=COLORS[0],
    ...
    """Barras horizontais com Top-N e proteção contra gráficos gigantes."""
    if table.empty:
        return
    n = count_limit(label, key, default_n)
    requested_all = n is None
    if requested_all and len(table) > MAX_PLOT_CATEGORIES:
        force = st.checkbox(
            f"Renderizar todas as {fmt_int(len(table))} categorias (pode ficar pesado)",
            value=False, key=f"force_all::{key}",
        )
        n = None if force else MAX_PLOT_CATEGORIES
    plot = top_n(table, n).sort_values(value)
    st.caption(f"Gráfico: {fmt_int(len(plot))} de {fmt_int(len(table))} categorias. "
               "A tabela/download preserva todas as categorias.")
...
st.caption(
    ...
    "curadoria DPOL. Os gráficos têm controle de Top N ou todas as categorias."
)
...
def page_overview():
    ...
    if len(interest_counts):
        fig = px.bar(interest_counts, x="Registros", y="Grupo", orientation="h",
                     color="Grupo", color_discrete_sequence=COLORS,
...
def page_interest():
    ...
    fig = px.bar(
        ...
        color="Grupo", color_discrete_sequence=COLORS, text="Registros_filtrados",
    ...
    if len(by_host):
        ...
        cap = min(len(totals), MAX_PLOT_CATEGORIES) if n is None else n
        keep = totals.head(cap).index
        by_host_plot = by_host[by_host[host_col].isin(keep)]
        fig = px.bar(
            by_host_plot, x="Registros", y=host_col, color="interest_group",
            orientation="h", barmode="stack", color_discrete_sequence=COLORS,
...
def page_taxon_explorer():
    ...
    mode = st.radio("Usar", ["species_final", "organism"], horizontal=True)
    taxa_options = sorted(known(records, mode)[mode].astype(str).unique().tolist())
    selected = st.multiselect(
        "Selecione um ou mais táxons", taxa_options, max_selections=20, key="explorer_taxa"
    )
    if not selected:
        st.info("Selecione um ou mais táxons para iniciar a exploração.")
        return

    explorer = filtered_records[filtered_records[mode].isin(selected)]
    if explorer.empty:
        st.info("Os táxons selecionados não têm registros com os filtros atuais.")
        return

    e1, e2, e3, e4 = st.columns(4)
    e1.metric("Registros", fmt_int(len(explorer)))
    e2.metric("Países", fmt_int(known(explorer, "country_canonical")["country_canonical"].nunique()))
    e3.metric("Hospedeiros", fmt_int(known(explorer, "host_final")["host_final"].nunique()))
    e4.metric("Anos", fmt_int(explorer["collection_year_num"].dropna().nunique()))
    ...
    by_year = group_count(
        explorer[explorer["collection_year_num"].notna()], ["collection_year_num", mode]
    )
    if len(by_year):
        ...
        if len(groups) > MAX_LINE_GROUPS:
            default_line = groups[:MAX_LINE_GROUPS]
            line_groups = st.multiselect(
                f"Táxons na série temporal (máx. {MAX_LINE_GROUPS})",
                groups, default=default_line, max_selections=MAX_LINE_GROUPS,
                key="explorer_line_taxa",
            )
            st.caption(
                f"A seleção contém {fmt_int(len(groups))} táxons; a série temporal limita a "
                f"{MAX_LINE_GROUPS} para evitar repetição de cores. A tabela continua completa."
            )
    ...
    ranked_bar(
        ...
        title="Hospedeiros dos táxons selecionados", color=COLORS[2],
    )
    ...
    deferred_download(
        ...
        "taxon_explorer", "dl_explorer", extra=f"{mode}|{selected}",
    )
...
def page_geo():
    ...
    ranked_bar(
        ...
        label="Países no gráfico de registros", title="Registros por país (nomes canônicos)",
    )

    mapped, unmapped = L.add_iso3(countries_all, "País", "Registros")
    if len(mapped):
        fig = px.choropleth(
            mapped, locations="iso3", locationmode="ISO-3", color="Registros", hover_name="País",
            color_continuous_scale="Cividis", title="Mapa mundial de registros",
        )
        show_fig(fig, 600)
    st.caption(
        "O ranking usa uma camada canônica conservadora para aliases conhecidos "
        "(ex.: Turkiye→Turkey e Zaire→RD Congo). Termos ambíguos como “Korea” não são forçados para South Korea; country_final original é preservado."
    )
    if unmapped:
...
def page_brazil():
    ...
    if len(br_interest):
        ...
        fig = px.bar(
            ...
            color_discrete_sequence=COLORS, text="Registros", log_x=(scale_mode == "Logarítmica"),
        )
...
def page_hosts():
    ...
    if crosswalk_loaded:
        st.caption(
            "host_final usa um crosswalk curado por correspondência exata; o campo host original permanece disponível para auditoria."
        )
    else:
        st.caption(
    ...
    n_effective = min(MAX_PLOT_CATEGORIES, len(host_counts_all)) if n is None else n
...
def page_hypothetical():
    ...
    if n is None and len(summary) > MAX_PLOT_CATEGORIES:
        st.caption(f"'Todas' excede {MAX_PLOT_CATEGORIES} categorias; o gráfico é limitado para preservar o navegador.")
        n = MAX_PLOT_CATEGORIES
    ...
    if len(plot):
        ...
        show_fig(fig, categorical_height(len(plot)))
        st.caption(f"Gráfico: {fmt_int(len(plot))} de {fmt_int(len(summary))} grupos elegíveis após o mínimo de CDS.")
...
def page_quality():
    ...
    st.caption(
        "A coluna global é recalculada diretamente de records_dashboard com a mesma regra de valores ausentes, "
        "evitando discrepâncias históricas do metadata_quality.tsv (por exemplo, o texto literal 'missing')."
    )
...
def page_provenance():
    ...
    st.info(
        "Os arquivos originais não são reescritos por esta camada. As correções de plotagem são "
        "campos derivados/auditáveis. A integridade local é verificada por contagens e SHA256; a "
        "atualidade taxonômica pode ser verificada separadamente contra o NCBI."
    )
    ...
    if dataset_manifest:
        ...
        st.caption(
            "Política: os arquivos-fonte de registros/CDS não são reescritos; country_canonical é apenas "
            "camada de plotagem; host não é normalizado sem crosswalk curado; validação NCBI não altera o arquivo arquivado."
        )
        with st.expander("Ver manifest completo"):
            st.json(dataset_manifest)
    else:
        st.caption("dataset_manifest_v7.json não encontrado; usando apenas BUILD_AUDIT.")
...
def page_dpol():
    ...
    if len(flow):
        colors = [COLORS[i % len(COLORS)] for i in range(len(flow))]
        fig = go.Figure(go.Funnel(y=flow["stage"], x=flow["n"], textinfo="value",
...
def page_records():
    ...
    if query:
        table = table[L.search_mask(table, query, [
            ...
            "country_final", "country_canonical", "host", "host_final", "host_group", "isolate", "strain",
        ])]
    ...
    cols = show_table(
        ...
        ["accession_version", "taxid", "definition", "length_nt", "sequence_status", "organism",
         "species_final", "interest_group", "taxonomy_final_status", "country_final", "country_canonical",
...
# ============================================================
# NAVEGAÇÃO (só a página escolhida é executada)
# ============================================================

pages = {
    "Explorar": [
        st.Page(page_overview, title="Visão geral", icon="🏠", url_path="visao-geral", default=True),
        st.Page(page_interest, title="Espécies de interesse", icon="⭐", url_path="interesse"),
        st.Page(page_taxon_explorer, title="Explorador taxonômico", icon="🔎", url_path="taxonomia"),
    ],
    "Geografia e tempo": [
        st.Page(page_geo, title="Geografia", icon="🌍", url_path="geografia"),
        st.Page(page_time, title="Tempo", icon="📅", url_path="tempo"),
        st.Page(page_brazil, title="Brasil", icon="🇧🇷", url_path="brasil"),
    ],
    "Biologia": [
        st.Page(page_hosts, title="Hospedeiros", icon="🐒", url_path="hospedeiros"),
        st.Page(page_genes, title="Genes / CDS", icon="🧫", url_path="genes-cds"),
        st.Page(page_hypothetical, title="Proteínas hipotéticas", icon="❓", url_path="hipoteticas"),
        st.Page(page_compare, title="Comparação de proteínas", icon="🧬", url_path="comparacao"),
    ],
    "Dados": [
        st.Page(page_quality, title="Qualidade", icon="✅", url_path="qualidade"),
        st.Page(page_provenance, title="Proveniência / NCBI", icon="🔐", url_path="proveniencia"),
        st.Page(page_dpol, title="DPOL / Curadoria", icon="🧪", url_path="dpol"),
        st.Page(page_records, title="Registros", icon="📋", url_path="registros"),
    ],
}
...
st.caption(
    ...
    "Representação no dataset corresponde ao número de registros NCBI/GenBank e não deve ser "
    "interpretada como prevalência biológica. species_final preserva a classificação normalizada; "
    "organism e host preservam os valores originais; country_canonical/host_final são camadas de "
    "agregação auditáveis. Paleta Okabe-Ito/Cividis escolhida para melhor distinção em públicos com "
